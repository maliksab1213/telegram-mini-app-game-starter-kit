// casino_games/ tigervsdragon.routes.js Tiger vs Dragon Game Routes
const { checkUserAccess } = require('../apis');

async function handleTigerVsDragonRoutes(req, res, parsed, botHandler) {
    ...
    const gameInstance = global.tigerVsDragonGame;

    // Get round info
    if (pathname === "/api/casino/tigervsdragon/round" && req.method === "GET") {
        try {
            ...
            console.error("[TigerVsDragon] Error getting round:", error);
    ...
    // Place bet
    if (pathname === "/api/casino/tigervsdragon/bet" && req.method === "POST") {
        let body = '';
        req.on('data', chunk => body += chunk.toString());

        req.on('end', async () => {
            try {
                const { position, amount } = JSON.parse(body);
                ...
                const result = gameInstance.placeBet(uid, position, amount);
    ...
    // Get user stats
    if (pathname === "/api/casino/tigervsdragon/stats" && req.method === "GET") {
        try {
            ...
            console.error("[TigerVsDragon] Error getting stats:", error);
            res.writeHead(500, { "Content-Type": "application/json" });
...
module.exports = handleTigerVsDragonRoutes;