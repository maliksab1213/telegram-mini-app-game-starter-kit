// casino_games/bankervs.routes.js - Banker vs Player Game Routes
const { checkUserAccess } = require('../apis');

async function handleBankerVsRoutes(req, res, parsed, botHandler) {
    ...
    const gameInstance = global.bankerVsGame;

    // Get round info
    if (pathname === "/api/casino/bankervs/round" && req.method === "GET") {
        try {
            ...
            console.error("[BankerVs] Error getting round:", error);
    ...
    // Place bet
    if (pathname === "/api/casino/bankervs/bet" && req.method === "POST") {
        let body = '';
    ...
    // Get user stats
    if (pathname === "/api/casino/bankervs/stats" && req.method === "GET") {
        try {
            ...
            console.error("[BankerVs] Error getting stats:", error);
            res.writeHead(500, { "Content-Type": "application/json" });
...
module.exports = handleBankerVsRoutes;